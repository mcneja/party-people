...
type RenderLitColored = (
    compiledGeom: CompiledGeom,
    matScreenFromWorld: mat4,
    matWorldFromLocal: mat4,
    lighting: Lighting,
    color: vec3) => void;
...
function createRenderer(gl: WebGL2RenderingContext, fontImage: HTMLImageElement): Renderer {
    ...
    function setDepthTestEnabled(enabled: boolean) {
        if (enabled) {
...
function renderPerson(renderer: Renderer, lighting: Lighting, matScreenFromWorld: mat4, person: PersonRenderState) {
...
function renderScene(renderer: Renderer, state: State) {
    ...
    const personRenderState: PersonRenderState = {
